...
void SimpleRISCVCPU::execute(const Instruction& inst) {
    if (inst.op == "ADD") {
        writeReg(inst.rd, regs[inst.rs1] + regs[inst.rs2]); // Week 3: verify this behaviour
    } else if (inst.op == "SUB") {
        writeReg(inst.rd, regs[inst.rs1] - regs[inst.rs2]); // Week 4: debug this if a test fails
