...
std::string SimpleRISCVCPU::execute(const Instruction& inst) {
    ...
    if (op == "ADD") {
        int32_t a = readReg(inst.rs1), b = readReg(inst.rs2), r = a + b; // Week 3: verify this behaviour
        writeReg(inst.rd, r);
        d << "ADD x" << inst.rd << ", x" << inst.rs1 << ", x" << inst.rs2
          << "  | " << wrote(inst.rd, std::to_string(a) + " + " + std::to_string(b) +
                             " = " + std::to_string(r));
    } else if (op == "SUB") {
        int32_t a = readReg(inst.rs1), b = readReg(inst.rs2), r = a - b; // Week 4: debug this if a test fails
        writeReg(inst.rd, r);
